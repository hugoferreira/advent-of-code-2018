...
const ready = _.pipe(_.rejectValues(_.isNotEmpty), _.sortKey, _.keys)
const times = new Map(nodes.map(e => [e, e.charCodeAt(0) - 5]))
...
while (_.isNotEmpty(work) || _.isNotEmpty(next)) {
    const idle = nworkers - work.length
    work = _.concat(work, _.take(idle)(next))
    next = _.drop(idle)(next)
